...
bool WinUsbTransport::open() {
    ...
    HDEVINFO devInfo = SetupDiGetClassDevsW(&kPicoJ2534InterfaceGuid, nullptr, nullptr, DIGCF_PRESENT | DIGCF_DEVICEINTERFACE);
    if (devInfo == INVALID_HANDLE_VALUE) {
        setError("SetupDiGetClassDevs failed for Pico J2534 WinUSB interface");
        return false;
    }
    ...
    for (DWORD index = 0; SetupDiEnumDeviceInterfaces(devInfo, nullptr, &kPicoJ2534InterfaceGuid, index, &ifData); ++index) {
        DWORD needed = 0;
